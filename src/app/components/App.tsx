...
export default function App({ session, cookie }: any) {
  ...
  // @ts-ignore
  const [clickField, setClickField] = useState<ClickField>({
    type: '',
    field: '',
  })
    ...
    if (clickField.field.length > 0) {
      if (clickField.field && (clickField.type || clickField.data.queryField)) {
        ...
        if (clickField.data.queryField) {
          updateQueryAsObj(
            '',
            clickField.field.toLowerCase(),
            clickField.data.arguments,
          )
        }
    ...
    <>
      <SessionProvider session={session}>
        ...
        <SaveModal
          ...
          setIsSaveModalOpen={setIsSaveModalOpen}
          setIsSaveResponseModalOpen={setIsSaveResponseModalOpen}
          setSaveResponseStatus={setSaveResponseStatus}
          query={queryAsString}
          endpoint={data.endpoint}
        />
        <SaveResponseModal
          isSaveResponseModalOpen={isSaveResponseModalOpen}
          setIsSaveResponseModalOpen={setIsSaveResponseModalOpen}
