...
export default function createNewNodesAndEdges(
  schema: SchemaData,
  setArgumentFunc: Dispatch<SetStateAction<number | string>>,
) {
  console.log(schema)
  const newDataForFlow: { newNodes: Node[]; newEdges: Edge[] } = {
    newNodes: [],
    newEdges: [],
  }
  ...
  schema.fields.map((field: any) => {
    ...
    let height
    const fieldArgLength = field.reqArgs.length
    ...
    for (const el of schema.types[field.type]) {
      if (field.reqArgs.includes(el.name)) continue
      console.log(el.isObject)
      const newTypeFieldNode: Node = {
