...
describe("e2e Test for Vista", () => {
  ...
  it("Should update the query generator based on the fields clicked and run the query", () => {
    ...
    cy.get('[data-testid="rf__node-id_argument_character_parent"]').children('div').children('div.flex-1').children('input').type('1');
    ...
    cy.contains('store this query so you can reference it later').should('exist');
    
    const time = Date.now();
    const now = new Date(time);
    cy.get('[data-testid="saveQuery_queryName"]').type(`test-${now.toISOString()}`);
